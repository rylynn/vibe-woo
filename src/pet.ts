...
export class Pet {
   ...
   * 作废绘制缓存，强制下一帧重画。
   *
   * 访客宠物和主宠物共用同一张画布：访客擦自己的脏矩形时可能擦到
   * 主宠物身上，而主宠物此刻可能因为「视觉指纹未变」直接跳帧 ——
   * 身上就留一个洞，要等它自己表情变了才补上。访客画完调一下这个，
   * 主宠物下一拍就会重画。没有副作用，也没有状态写回。
   */
  invalidate(): void {
    // 只作废视觉指纹 —— 让下一拍不再因为「看起来没变」而跳过绘制。
    //
    // 刻意**不清** dirty：上一帧画在哪儿这件事并没有变，脏矩形依然有效。
    // 清空它会让下一帧退化成整屏 clearRect（CLAUDE.md:81 明令禁止），
    // 而且那次整屏清除发生在访客绘制之前，会把访客整只擦掉。
    //
    // 也刻意**不动** lastRenderMs：帧预算到点自然会画，
    // 强抬到「立即」等于把主宠物钉在访客的帧率上，白烧 CPU。
    this.lastDrawKey = null;
  }
