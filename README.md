...
两个入口，都由**快捷键主动唤起**（不监听、不自动弹出）：

| 快捷键 | 作用 | 所需权限 |
|---|---|---|
| `Ctrl+Alt+T` | 读取当前前台应用里选中的文字 | 辅助功能（首次使用时按需申请） |
| `Ctrl+Alt+O` | 屏幕框选，本地识别画面里的文字 | 屏幕录制（首次使用时按需申请） |

取到文字后弹出浮窗：原文可编辑纠错，再点**翻译**（默认英译中，可改中译英）或**搜索**（Google / Bing / 百度可配，用系统默认浏览器打开）。

- **识别在本地完成**：框选的画面只交给系统 Vision 做文字识别，不上传、不落盘。
- **只有点翻译或搜索才外发**文本；翻译走的是你自己在设置里配置的 LLM 服务，搜索只构造受控搜索引擎地址。
- 单次文本上限 4000 字符；不保存原文、译文与历史。
- 两个快捷键都可在「设置 → 取词与翻译」里改；辅助功能未授权时仍可独立使用屏幕框选。
...
- **取词与翻译是「用户主动授权的工具」，与上面的后台感知刻意分开**（见 `src-tauri/src/texttools/`）：
  ...
  - 取词只在按下快捷键后读一次前台应用的**选区文本**：优先 `AXSelectedText`，控件不支持时用选区范围取串；不读取整个控件值、不递归扫描应用内容、不模拟复制、不碰剪贴板（复制是你点按钮才写）。
  - 框选截图用 ScreenCaptureKit 只取**单帧**，排除本应用窗口，收首帧即停流；图像只在内存里交给 Vision，不写临时文件。
  - 权限状态每次实时查询，不缓存为「已授权」；拒绝或撤销后只提示，不重试、不偷偷再申请。
