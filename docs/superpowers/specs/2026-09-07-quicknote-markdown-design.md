...
1. **首行行内代码被吞成 tag**：`for tok in rest.split_whitespace()` 从中间抽 `` `token` `` —— 用户首行写 `` 回邮件 `tomorrow` ``，roundtrip 后 `tomorrow` 变 tag、文本丢失。
   ...
   **实施裁决（Task 5 评审确认）**：行尾单个 token 与行内代码语法上不可区分（新旧行为期望互相矛盾），故收紧为**行尾连续 ≥2 个 token 才认作 tags，单个保守归内容**——数据无损原则下保护高频真实场景（行内代码），牺牲的「单 tag 读回」场景当前不存在：LLM enrich 的 tags 只经 `pet://note-enriched` 事件 emit（前端无监听者），**从不落盘**，生产文件行尾不会有 tag 后缀。若日后实现 tags 落盘回填，需同步引入可区分的 tags 标记（属存储格式变更，单独立项）。
...
已知边界（≥2 门槛的镜像代价，单 token 二义性固有、无启发式可解）：首行恰以 ≥2 个相邻行内代码结尾且其后无正文时会被误读为 tags（如 `` 对比 `a` `b` `` 读回 `text="对比"`、`tags=["a","b"]`，两个代码段从面板文本消失）——仅展示层损失，文件始终是真源不回写；罕见形态，可接受。
