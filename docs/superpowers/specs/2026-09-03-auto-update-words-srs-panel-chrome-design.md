...
- **升级气泡刻意绕过说话门槛**（talkdrive 人格频率 / 勿扰 / 仲裁器 / 番茄静默），也不受 `auto_update` 开关约束（2026-09-06 终审补记）：每版本仅一次、≤50 字、8 秒自动消失、纯本地触发，属「用户理应知道更新了什么」的系统级通知而非闲聊——「不打扰是第一原则」让位于「手动升级的用户必须得到交代」这一次性通知。后续若在更多 UI 接线更新状态：**必须监听 `pet://update-status` 事件**，不要 `await check_update_now`——找到更新时该 promise 会一直挂到安装重启（可能数小时），且永不返回 Err（失败全部走事件）。
