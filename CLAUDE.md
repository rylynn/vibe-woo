...
### 取词与翻译工具（src-tauri/src/texttools/）

**用户主动触发**的能力，与上面的零授权感知刻意分开：快捷键（默认 `Ctrl+Alt+O`，可在设置里改）→
会话建立 → 屏幕框选 OCR → 结果只定向发给 `pet` 窗口 → 默认自动翻译（设置可关）/ 用户点击搜索。
（2026-09-19：早期的原生 AX 选区取词入口因 TCC 授权在 ad-hoc 重签名后反复失效、整体下掉，只剩框选 OCR 一条路。）

- 会话门（`SessionGate`）保证重复触发、关闭面板后旧结果失效、迟到结果一律丢弃；
  前端 `src/text-tools.ts` 的 `SessionGuard` 再守一次。
- 框选层是独立的短生命周期 NSPanel，不复用宠物全屏窗；ScreenCaptureKit 单帧截图
  （排除本应用、收首帧即停流），Vision 本地识别中英文，图像不落盘。
- 「屏幕录制」授权引导：Rust 先 `tccutil reset` 清掉陈旧条目（只动自己），再
  `CGRequestScreenCaptureAccess` 让系统把应用加进列表，最后打开系统设置隐私页；
  用户只需勾选 + 重启应用。绝不后台轮询授权状态。
- 框选区域没识别到文字不算失败：前端给「待框选」引导与一键重新框选，不弹错误。
- 翻译复用用户已配置的 LLM：低温（0.2）、独立输出预算（8192 tokens）、响应长度上限，
  超上限报错而非静默截断；错误只回脱敏类别，不透传服务端响应体。
- 搜索用受控枚举 + 固定 HTTPS 地址，参数统一编码，超长拒绝。

## 红线与硬约束

1. **隐私红线不可放宽**：`share.rs` 的社交上报是白名单构造（从零构造新结构，不是从状态里删字段）；`sensor.rs` 只取时间间隔；`envsense.rs` 零授权零内容。改动这几处会被重点 review。
   - 取词工具（`texttools/`）是**主动授权**分支，不是后台感知：绝不能把它接进
     感知循环、插件调度、习惯记忆或社交上报；原文/译文/截图不得进入任何日志、配置或上报。
   - 日志只记阶段、耗时与错误类别，不记原文、译文、搜索完整 URL 或密钥。
...
- 新版本号落定的同一次推送，`src-tauri/version-notes.json` 必须带上该版本的 ≤50 字非空摘要（升级气泡文案源，`release.sh` 发版时校验，缺了会被拦）。
...
- 同步服务部署后验收：`pnpm test:remote <地址>`（如 `pnpm test:remote http://1.2.3.4:8787`）。走真实 HTTP 跑全链路，**会留下两个测试账号**，只对着刚部署的空库跑。
