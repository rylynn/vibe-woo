...
**用户主动触发**的能力，与上面的零授权感知刻意分开：快捷键 → 会话建立 + 来源快照 →
AX 选区读取 / 屏幕框选 OCR → 结果只定向发给 `pet` 窗口 → 默认自动翻译（设置可关）/ 用户点击搜索。

- 两个入口各一个快捷键（可在设置里改）：原生选区默认 `Ctrl+Alt+T`、框选 OCR 默认 `Ctrl+Alt+O`。
- 会话门（`SessionGate`）保证重复触发、关闭面板后旧结果失效、迟到结果一律丢弃；
  前端 `src/text-tools.ts` 的 `SessionGuard` 再守一次。
- 取词优先读 `AXSelectedText`，控件不支持时用选区范围取串；不读整个控件值、不递归扫描、
  不模拟复制、不后台读剪贴板（复制按钮才写）。
